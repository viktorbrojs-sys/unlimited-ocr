...
- **CUDA out of memory при загрузке** (карты на 8 ГБ): bf16-веса (~6.7 ГБ)
  ...
  по цепочке: AWQ 4-бит → bf16 → CPU (см. переключатель варианта модели в
  шапке страницы). Bitsandbytes (8/4-бит) не используется — не совместим
  с архитектурой модели (MoE + R-SWA). Принудительный CPU (медленно):
